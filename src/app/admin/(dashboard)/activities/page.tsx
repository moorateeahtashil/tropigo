...
import { listActivities, deleteActivity } from './actions'

export const dynamic = 'force-dynamic'

export default async function ActivitiesList({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  const resolvedSearchParams = await searchParams
  const q = resolvedSearchParams.q || ''
  const rows = await listActivities(q)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-ink-secondary">Manage activities in the catalog.</p>
        </div>
        <Link href="/admin/activities/new" className="rounded-xl bg-brand-700 px-3 py-2 text-sm font-medium text-white shadow-sm hover:bg-brand-800">New Activity</Link>
      </div>
            ...
            {rows.map((r: any) => (
              <tr key={r.id} className="border-t border-sand-100">
                ...
                <td className="px-4 py-2 text-right">
                  <Link href={`/admin/activities/${r.id}`} className="text-brand-700 hover:underline">Edit</Link>
                  <form action={deleteActivityAction} className="ml-3 inline">
