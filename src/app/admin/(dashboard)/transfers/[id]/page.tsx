    ...
    <form action={saveAction} className="space-y-6">
      ...
      <section className="rounded-2xl border border-sand-200 bg-white p-5 shadow-card">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <Field label="From Price (optional)"><input name="base_price" type="number" step="0.01" defaultValue={row.base_price ?? ''} min="0" className="w-full rounded-lg border-sand-300 shadow-sm focus:border-brand-500 focus:ring-brand-500" /></Field>
          <Field label="Status">
      ...
      <section className="rounded-2xl border border-sand-200 bg-white p-5 shadow-card">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <Field label="Pricing Model">
            <select name="pricing_model" defaultValue={t?.pricing_model ?? 'fixed'} className="w-full rounded-lg border-sand-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">
          ...
          <Field label="Vehicle Type">
            <select name="vehicle_type" defaultValue={t?.vehicle_type ?? 'sedan'} className="w-full rounded-lg border-sand-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">
          ...
          <Field label="Max Passengers"><input name="max_passengers" type="number" defaultValue={t?.max_passengers ?? 4} min={1} className="w-full rounded-lg border-sand-300 shadow-sm focus:border-brand-500 focus:ring-brand-500" /></Field>
          <Field label="Max Luggage"><input name="max_luggage" type="number" defaultValue={t?.max_luggage ?? ''} min={0} className="w-full rounded-lg border-sand-300 shadow-sm focus:border-brand-500 focus:ring-brand-500" /></Field>
          <Field label="Includes Meet & Greet"><input name="includes_meet_greet" type="checkbox" defaultChecked={t?.includes_meet_greet} className="h-4 w-4 rounded border-sand-300" /></Field>
          <Field label="Includes Flight Tracking"><input name="includes_flight_tracking" type="checkbox" defaultChecked={t?.includes_flight_tracking} className="h-4 w-4 rounded border-sand-300" /></Field>
          <Field label="Base Fare"><input name="base_fare" type="number" step="0.01" defaultValue={t?.base_fare ?? ''} min="0" className="w-full rounded-lg border-sand-300 shadow-sm focus:border-brand-500 focus:ring-brand-500" /></Field>
          <Field label="Per KM Rate"><input name="per_km_rate" type="number" step="0.0001" defaultValue={t?.per_km_rate ?? ''} min="0" className="w-full rounded-lg border-sand-300 shadow-sm focus:border-brand-500 focus:ring-brand-500" /></Field>
          <Field label="Waiting Fee / Hour"><input name="waiting_fee_per_hour" type="number" step="0.01" defaultValue={t?.waiting_fee_per_hour ?? ''} min="0" className="w-full rounded-lg border-sand-300 shadow-sm focus:border-brand-500 focus:ring-brand-500" /></Field>
          <Field label="Notes" full><textarea name="notes" defaultValue={t?.notes ?? ''} rows={2} className="w-full rounded-lg border-sand-300 shadow-sm focus:border-brand-500 focus:ring-brand-500" /></Field>
        </div>
      </section>
    ...
    <section className="rounded-2xl border border-sand-200 bg-white p-5 shadow-card">
      ...
      <form action={upsertZoneAction.bind(null, transferId)} className="grid gap-3 sm:grid-cols-5">
        <div>
          ...
          <select name="from_zone_id" className="w-full rounded-lg border-sand-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">
            {zones.map(z => <option key={z.id} value={z.id}>{z.name}</option>)}
          </select>
        </div>
        <div>
          <label className="mb-1 block text-sm font-medium text-ink">To Zone</label>
          <select name="to_zone_id" className="w-full rounded-lg border-sand-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">
            {zones.map(z => <option key={z.id} value={z.id}>{z.name}</option>)}
          </select>
        </div>
        <div>
        ...
        <div>
          ...
          <input type="number" name="price" step="0.01" min="0" className="w-full rounded-lg border-sand-300 shadow-sm focus:border-brand-500 focus:ring-brand-500" />
        </div>
