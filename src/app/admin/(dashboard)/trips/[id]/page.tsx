...
export default async function EditTrip({ params }: { params: Promise<{ id: string }> }) {
  ...
  const productDestinations = trip.product_destinations ?? []
  const destinationId = productDestinations[0]?.destination_id ?? trip.destination_id ?? ''

  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <h1 className="text-2xl font-semibold text-ink">Edit Trip</h1>
      <TripForm
        trip={trip}
...
function TripForm({
  ...
  trip: Awaited<ReturnType<typeof getTripProduct>>
  destinationId: string
  destinations: Array<{ id: string; name: string; region: string | null }>
  productMedia: Array<{ id: string; url: string; alt: string | null; is_cover: boolean; sort_order: number; media_type: string }>
}) {
  const product = trip
  ...
  return (
    <>
      <form action={updateAction.bind(null, product.id)} className="space-y-6">
        ...
        {/* Itinerary */}
        <section className="rounded-2xl border border-sand-200 bg-white p-5 shadow-card">
          <h2 className="mb-3 text-lg font-semibold text-ink">Itinerary (JSON)</h2>
        ...
        <div className="flex gap-2">
          <button className="rounded-xl bg-brand-700 px-6 py-2.5 font-medium text-white shadow-sm hover:bg-brand-800">Save Changes</button>
        </div>
      </form>

      {/* Delete */}
      <form action={deleteAction.bind(null, product.id)}>
        <button className="rounded-xl border border-red-300 bg-white px-6 py-2.5 font-medium text-red-600 shadow-sm hover:bg-red-50">
          Delete Trip
        </button>
      </form>
    </>
