...
import { listPackages, deletePackage } from './actions'
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-ink-secondary">Manage multi-activity packages.</p>
        </div>
        <Link href="/admin/packages/new" className="rounded-xl bg-brand-700 px-3 py-2 text-sm font-medium text-white shadow-sm hover:bg-brand-800">New Package</Link>
      </div>
            ...
            {rows.map((r: any) => (
              <tr key={r.id} className="border-t border-sand-100">
                ...
                <td className="px-4 py-2 text-right">
                  <Link href={`/admin/packages/${r.id}`} className="text-brand-700 hover:underline">Edit</Link>
                  <form action={deletePackageAction} className="ml-3 inline">
