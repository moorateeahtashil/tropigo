import { createActivity, listDestinationsOptions } from '../actions'

export default async function NewActivity() {
  const destinations = await listDestinationsOptions()
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <h1 className="text-2xl font-semibold text-ink">New Activity</h1>
      <ActivityForm destinations={destinations} />
    </div>
  )
}

function ActivityForm({ destinations }: { destinations: Array<{ id: string; name: string; region: string | null }> }) {
  return (
...
async function createAction(formData: FormData) {
  ...
  await createActivity(product as any, activity as any)
}
